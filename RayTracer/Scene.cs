...
namespace RayTracer
{
    public class Scene
    {
        ...
        public Scene(int sizeX, int sizeY, double pixelSize, ICamera sceneCamera, Color ambientLight, List<Primitive> sceneObjects, List<Light> sceneLights, IShader sceneShader, Color backgroundColor)
        {
            SizeX = sizeX;
            SizeY = sizeY;
        ...
        public View ParallelRender()
        {
            var result = SceneCamera.GetView(SizeX, SizeY, PixelSize);
            var renderedPixels = new ConcurrentDictionary<(int x, int y), View.Point>();
            ...
            Parallel.ForEach(points, target =>
            {
                var targetPixel = result.Pixels[target.x, target.y].realCoordinate;

                var ray = SceneCamera.GenerateRay(targetPixel);

                var point = RayTrace(ray, 1, level);

                renderedPixels.TryAdd(target, point);
            });
            ...
            for (int y = 0; y < SizeY; y++)
            {
                for (int x = 0; x < SizeX; x++)
                {
                    result.Pixels[x, y] = renderedPixels[(x, y)];
                }
            }
